      ...
      <div class="flex gap-6">
        ...
        <main class="flex-1 min-w-0">
          ...
          <MonitorConfig
            ...
            @select-market-type="selectMarketType"
          ...
          <!-- 信号质量控制已整合到 IndicatorAlertConfig 内，无需单独显示 -->
...
// 简化：移除 SignalTypeSelector、PriceAlertConfig、VolatilitySignalConfig、VolumeSignalConfig
// 只保留 indicator_alert 类型
...
const formData = ref({
  ...
  trading_pair: 'USDT',
  timeframe: '1h',
  market_type: 'spot',  // 市场类型：spot=现货, futures=合约
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const submitData = {
      ...
      market_type: formData.value.market_type,  // 市场类型
      signal_type: 'indicator_alert',  // 简化：固定为 indicator_alert
...
const selectMarketType = (marketType) => {
  // 如果市场类型没变，不做任何操作
  if (formData.value.market_type === marketType) {
    return
  }
  ...
  if (selectedToken.value) {
    ...
    if (marketType === 'spot' && !token.is_spot_available) {
      // 代币不支持现货，清空选择
      showError(`代币 ${token.symbol} 不支持现货交易，已清空选择`)
      clearTokenSelection()
    } else if (marketType === 'futures' && !token.is_futures_available) {
      // 代币不支持合约，清空选择
      showError(`代币 ${token.symbol} 不支持合约交易，已清空选择`)
      clearTokenSelection()
    }
  }
...
const handleTokenSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      // 添加交易所和市场类型参数，只搜索该交易所和市场类型支持的代币
      // 市场类型映射：信号机器人的 futures -> API 的 linear
      const apiMarketType = formData.value.market_type === 'futures' ? 'linear' : formData.value.market_type
      const response = await apiRequest(
        `${API_ENDPOINTS.AI_STRATEGY_SEARCH_TOKEN}?q=${query}&exchange=${formData.value.exchange_name}&market_type=${apiMarketType}`
      )
      ...
      if (response.status === 'success') {
        ...
        if (tokenSearchResults.value.length === 0) {
          console.log(`⚠️ 在 ${formData.value.exchange_name} (${formData.value.market_type}) 上未找到 "${query}"`)
        }
      } else {
        tokenSearchResults.value = []
      }
...
const loadAccountTokens = async () => {
  ...
  try {
    ...
    for (const symbol of tokenSymbols) {
      try {
        const response = await apiRequest(
          `${API_ENDPOINTS.AI_STRATEGY_SEARCH_TOKEN}?q=${symbol}&exchange=${formData.value.exchange_name}&market_type=${apiMarketType}`
        )
...
const quickSelectToken = async (symbol) => {
  ...
  try {
    ...
    if (response.status === 'success' && response.data.results && response.data.results.length > 0) {
      ...
      showError(`在 ${formData.value.market_type === 'spot' ? '现货' : '合约'} 市场未找到 ${symbol}`)
    }
  } catch (error) {
...
const autoGeneratedDescription = computed(() => {
  ...
  const marketTypeLabel = formData.value.market_type === 'spot' ? '现货' : '合约'
  if (selectedToken.value && selectedToken.value.symbol) {
    parts.push(`监控 ${selectedToken.value.symbol}/${formData.value.trading_pair || 'USDT'} ${marketTypeLabel}交易对`)
  }
...
// 简化：移除 price_alert 相关的 watch（所有信号都是 indicator_alert）
...
const loadBotData = async () => {
  try {
    ...
    // 加载市场类型（优先从 signal_bot 获取，兼容 API 返回格式）
    formData.value.market_type = bot.signal_bot?.market_type || bot.market_type || 'spot'
...
const autoSelectToken = async (symbol) => {
  try {
    tokenSearching.value = true
    // 先尝试在默认交易所（gate）和当前市场类型上搜索
    const defaultExchange = 'gate'
    const apiMarketType = formData.value.market_type === 'futures' ? 'linear' : formData.value.market_type
    const response = await apiRequest(
      `${API_ENDPOINTS.AI_STRATEGY_SEARCH_TOKEN}?q=${symbol}&exchange=${defaultExchange}&market_type=${apiMarketType}`
    )
