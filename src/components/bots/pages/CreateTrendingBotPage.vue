...
const autoGeneratedDescription = computed(() => {
  ...
  if (selectedExchangeAPI.value && selectedToken.value?.symbol && formData.value.trading_pair) {
    ...
    parts.push(`${exchangeName} ${selectedToken.value.symbol}/${formData.value.trading_pair} ${marketType}`)
  }

  // 杠杆（合约才显示）
  if (formData.value.market_type !== 'spot' && formData.value.leverage) {
    parts.push(`${formData.value.leverage}x杠杆`)
  }
  ...
  // 止损
  if (formData.value.stop_loss_enabled && formData.value.stop_loss_config?.value) {
    parts.push(`止损${formData.value.stop_loss_config.value}%`)
  }

  // 止盈
  if (formData.value.take_profit_enabled && formData.value.take_profit_config) {
    if (formData.value.take_profit_config.mode === 'single' && formData.value.take_profit_config.single_value) {
      parts.push(`止盈${formData.value.take_profit_config.single_value}%`)
    } else if (formData.value.take_profit_config.targets?.length > 0) {
      parts.push(`${formData.value.take_profit_config.targets.length}级止盈`)
    }
  }

  // 追踪止损
  if (formData.value.trailing_stop_enabled && formData.value.trailing_stop_config?.callback_rate) {
    parts.push(`追踪止损${formData.value.trailing_stop_config.callback_rate}%`)
  }

  // 盈亏平衡
  if (formData.value.breakeven_enabled && formData.value.breakeven_trigger_percent) {
    parts.push(`盈亏平衡${formData.value.breakeven_trigger_percent}%`)
  }

  // 自动反向
  if (formData.value.auto_reverse) {
    parts.push('自动反向')
  }

  // 冷却期
  if (formData.value.cooldown_period) {
    parts.push(`冷却${formData.value.cooldown_period}分钟`)
  }

  // 每日最大交易
  if (formData.value.max_daily_trades) {
    parts.push(`日限${formData.value.max_daily_trades}次`)
  }

  // 最大持仓时间
  if (formData.value.max_position_time) {
    parts.push(`持仓上限${formData.value.max_position_time}小时`)
  }
